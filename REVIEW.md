...
7. **Hook Ledger.** Confirm the target ExecPlan includes a `Hook Ledger` with complete gate-attempt history for ledger-backed lifecycle events (`execplan.post-creation`, `execplan.resume`, and `execplan.post-completion` when attempted) and every event referenced by `hook_events`. Do not require `execplan.pre-creation` in the ledger because it runs before the plan file exists.
