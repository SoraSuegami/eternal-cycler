...
- This hook does not verify whether implementation commits were pushed. In loop-managed execution, the builder runs this hook before it returns success; the loop then only verifies the resulting completed plan and performs one final commit/push without invoking further builder edits.
